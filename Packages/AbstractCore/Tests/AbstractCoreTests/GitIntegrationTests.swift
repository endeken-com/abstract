...
private func makeRepo(_ exec: any Executor, _ label: String) async throws -> TempRepo {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("abstract-test-\(label)-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    let repo = TempRepo(root: dir.path)
    _ = try await git(exec, repo.root, ["init", "-q", "-b", "main"])
    _ = try await git(exec, repo.root, ["config", "user.email", "test@abstract.local"])
    _ = try await git(exec, repo.root, ["config", "user.name", "Abstract Test"])
...
@Suite struct GitIntegrationTests {
    ...
    @Test func worktreeDiffAcceptAndReject() async throws {
        ...
        try await Git.addWorktree(exec, root: repo.root, path: wtPath, branch: "abstract/test-session", baseRef: "HEAD")
        #expect(FileManager.default.fileExists(atPath: wtPath + "/app.txt"))
        #expect(try await Git.currentBranch(exec, root: wtPath) == "abstract/test-session")
        #expect(await Git.branchExists(exec, root: repo.root, branch: "abstract/test-session"))
        #expect(await !Git.branchExists(exec, root: repo.root, branch: "abstract/nope"))

        let listed = try await Git.worktrees(exec, root: repo.root)
        #expect(listed.count == 2, "main working tree plus the new worktree")
        #expect(listed.contains { $0.branch == "abstract/test-session" })
        ...
        try await Git.removeWorktree(exec, root: repo.root, path: wtPath, deleteBranch: "abstract/test-session")
    ...
    @Test func repoRootAndAttachingToAnExistingBranch() async throws {
        ...
        _ = try await git(exec, repo.root, ["branch", "abstract/again"])
        try await Git.addWorktree(exec, root: repo.root, path: wtPath, branch: "abstract/again", baseRef: "")
        #expect(try await Git.currentBranch(exec, root: wtPath) == "abstract/again")
    ...
    @Test func parsesPorcelainWorktreeList() {
        ...
        #expect(list[2].isLocked && list[2].branch == "abstract/x")
