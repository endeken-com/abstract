...
@Suite struct LocalExecutorTests {
    ...
    @Test func runMissingCommandThrowsNotFound() async {
        await #expect(throws: AbstractError.notFound("abstract-no-such-binary")) {
            try await self.exec.run("abstract-no-such-binary", [], cwd: nil)
        }
    }
    ...
    @Test func whichResolvesAgainstTheLoginPath() async throws {
        ...
        #expect(await exec.which("abstract-no-such-binary") == nil)
    ...
    func closedStdinLetsCatExit() async throws {
        ...
        #expect(throws: AbstractError.self) { try process.write("late\n") }
    }
    ...
    @Test func spawnMissingBinaryThrowsNotFound() {
        #expect(throws: AbstractError.notFound("abstract-no-such-binary")) {
            _ = try self.spawn("abstract-no-such-binary", [])
        }
    }
