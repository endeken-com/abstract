...
@Suite struct RecurrenceTests {
    ...
    @Test func invalidInputIsAnErrorNotAPanic() throws {
        #expect(throws: BacktickError.self) {
    ...
    @Test func validateRejectsBadRulesWithADescriptiveError() {
        #expect(throws: BacktickError.message("Unsupported rule part BYSETPOS")) {
            try Schedule.validate(rrule: "FREQ=MONTHLY;BYDAY=MO;BYSETPOS=1", timezone: "UTC")
        }
        #expect(throws: BacktickError.message("Unsupported frequency YEARLY")) {
            try Schedule.validate(rrule: "FREQ=YEARLY", timezone: "UTC")
        }
        #expect(throws: BacktickError.message("Unknown timezone \"Mars/Olympus\"")) {
        ...
        for rule in invalid {
            #expect(throws: BacktickError.self, "\(rule)") { try Schedule.validate(rrule: rule, timezone: "UTC") }
        }
        #expect(throws: BacktickError.self) { try Schedule.validate(rrule: "FREQ=DAILY", timezone: "") }
    }
