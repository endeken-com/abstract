...
@Suite struct StoreTests {
    ...
    @Test func projectRoundTripsEveryField() throws {
        let project = Project(
            id: "p1", name: "Backtick", rootPath: "/Users/me/Backtick", defaultBaseRef: "origin/main",
    ...
    @Test func reconcileMarksOnlyActiveSessionsInterrupted() throws {
        ...
        for status in SessionStatus.allCases {
            ...
            if status.isActive {
                #expect(session.status == .errored)
                #expect(session.statusDetail == "Interrupted when Backtick quit")
            } else {
    ...
    @Test func fileStoreCreatesItsDirectoryAndPersistsAcrossReopen() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("backtick-store-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: root) }
        let path = root.appendingPathComponent("nested/dir/backtick.sqlite").path
    ...
    @Test func defaultPathEndsInBacktickSqlite() {
        let path = Store.defaultPath()
        if let dir = ProcessInfo.processInfo.environment["BACKTICK_DATA_DIR"], !dir.isEmpty {
            #expect(path.hasSuffix("/backtick.sqlite"))
        } else {
            #expect(path.hasSuffix("/Library/Application Support/Backtick/backtick.sqlite"))
        }
    }
}
