...
@Suite struct WorktreeNamingTests {
    ...
    @Test func templateExpandsEveryToken() {
        let out = WorktreeNaming.render(
            ...
            slug: "fix-login", branch: "backtick/fix-login", prefix: "backtick/")
        #expect(out == "/Users/w/.backtick/worktrees/api-ab12cd34/fix-login")
        let branch = WorktreeNaming.render(
            template: "{prefix}{slug}", home: "", repo: "", hash: "", slug: "fix-login", branch: "", prefix: "backtick/")
        #expect(branch == "backtick/fix-login")
        #expect(WorktreeNaming.defaultBranchPrefix == "backtick/")
    }
