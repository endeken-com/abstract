...
public final class LocalExecutor: Executor, Sendable {
    ...
    public func run(_ command: String, _ args: [String], cwd: String?) async throws -> ExecResult {
        ...
        guard let executable = Self.resolveExecutable(command, searchPath: path, home: homeDirectory, cwd: cwd) else {
            throw BacktickError.notFound(command)
        }
        ...
        let result = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<ExecResult, any Error>) in
            ...
            do {
                ...
                collector.fail(BacktickError.message("failed to run `\(command)`: \(error.localizedDescription)"))
            }
        }
    ...
    public func spawn(
        ...
        else { throw BacktickError.notFound(spec.command) }
        var isDirectory: ObjCBool = false
        guard FileManager.default.fileExists(atPath: spec.cwd, isDirectory: &isDirectory), isDirectory.boolValue else {
            throw BacktickError.message("failed to spawn `\(spec.command)`: working directory \(spec.cwd) does not exist")
        }
...
final class LocalProcess: RunningProcess, Sendable {
    ...
    init(child: Spawner.Child, killGrace: TimeInterval,
        ...
        self.onLine = onLine
        self.onExit = onExit
        self.queue = DispatchQueue(label: "dev.backtick.process.\(child.pid)")
        self.stdinQueue = DispatchQueue(label: "dev.backtick.process.\(child.pid).stdin")
    ...
    func write(_ text: String) throws {
        ...
        guard accepted else { throw BacktickError.message("stdin is closed") }
    }
...
enum Spawner {
    ...
    static func launch(executable: String, arguments: [String], environment: [String: String], cwd: String?,
        ...
        func wire(_ stream: Stream, target: Int32) throws -> Int32 {
            ...
            case .pipe:
                var fds: [Int32] = [-1, -1]
                guard pipe(&fds) == 0 else { throw BacktickError.message("pipe: \(String(cString: strerror(errno)))") }
        ...
        guard rc == 0 else {
            ...
            throw BacktickError.message("failed to spawn `\(executable)`: \(String(cString: strerror(rc)))")
        }
...
enum LoginShell {
    static let marker = "__BACKTICK_LOGIN_PATH__"
    ...
    private static let queue = DispatchQueue(label: "dev.backtick.login-shell", qos: .userInitiated)
