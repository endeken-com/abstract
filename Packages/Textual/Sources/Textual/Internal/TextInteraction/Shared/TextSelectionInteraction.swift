...
struct TextSelectionInteraction: ViewModifier {
    ...
    @Environment(TextSelectionCoordinator.self) private var coordinator: TextSelectionCoordinator?
      ...
      /// A group keeps one selection across all its texts; the coordinator
      /// would undo a selection that runs through several of them.
      @Environment(\.textSelectionGroup) private var group
      private var effectiveCoordinator: TextSelectionCoordinator? { group == nil ? coordinator : nil }
    #else
      private var effectiveCoordinator: TextSelectionCoordinator? { coordinator }
    #endif
          ...
          .overlayTextLayoutCollection { layoutCollection in
            Color.clear
              .onChange(of: AnyTextLayoutCollection(layoutCollection), initial: true) {
                model.setCoordinator(effectiveCoordinator)
