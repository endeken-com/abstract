    ...
    private func sections(_ project: Project) -> some View {
        if let main = list.main {
            WorktreeSection(title: "Main working tree") {
                WorktreeRow(entry: main, projectId: project.id, onRemove: nil)
            }
        }
        ...
        WorktreeSection(title: "Chat worktrees", count: list.chats.count) {
            if list.chats.isEmpty {
                ...
                ForEach(Array(list.chats.enumerated()), id: \.element.id) { i, entry in
                    ...
                    WorktreeRow(entry: entry, projectId: project.id, onRemove: { removing = entry })
                }
            }
        }
        ...
        if !list.abandoned.isEmpty {
            WorktreeSection(title: "Abandoned", count: list.abandoned.count, warning: true,
                            note: "No chat uses these. They're left over from deleted chats, or ones made outside Abstract; remove them to reclaim the disk space.") {
...
private struct WorktreeRow: View {
    ...
    /// Any linked worktree whose folder exists can take a new chat; the main
    /// tree can't, since accepted changes land there.
    private var canStartChat: Bool {
        if case .main = entry.kind { return false }
        return entry.exists
    }
    ...
    var body: some View {
        HStack(spacing: Space.md) {
            ...
            HStack(spacing: 2) {
                if canStartChat {
                    Button { model.showNewChat(in: projectId, worktree: entry.path) } label: { Image(systemName: "plus.bubble") }
                        .buttonStyle(.icon)
                        .help(entry.session == nil ? "New chat here" : "New chat here (takes it over from “\(entry.session!.name)”)")
                }
        ...
        .contextMenu {
            if canStartChat {
                Button("New Chat Here") { model.showNewChat(in: projectId, worktree: entry.path) }
                Divider()
            }
    ...
    private var icon: some View {
        Group {
            switch entry.kind {
            case .main:
                Image(systemName: "house").foregroundStyle(Color.btAccent)
            case .chat(let s):
        ...
        VStack(alignment: .leading, spacing: Space.lg) {
            HStack(alignment: .top, spacing: Space.md) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Abstract deletes the folder below. Anything uncommitted in it is lost; commits stay on the branch unless you delete it too.")
                        .font(.btCallout)
                        .foregroundStyle(Color.btTextSecondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
