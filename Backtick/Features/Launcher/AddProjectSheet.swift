    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Space.lg) {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Point Backtick at a git repository on this Mac.").font(.btCallout).foregroundStyle(Color.btTextSecondary)
            }
            ...
            Button(action: choose) {
                HStack(spacing: Space.md) {
                    Image(systemName: probe == nil ? "folder" : "folder.fill")
                        .font(.system(size: 20))
                        .foregroundStyle(probe == nil ? Color.btTextSecondary : Color.accentColor)
                        .frame(width: 28)
                    VStack(alignment: .leading, spacing: 2) {
            ...
            if let probe, !probe.isRoot {
                Notice(text: "That folder sits inside a larger repository, so Backtick uses its root. Worktrees then carry the whole project.")
            }
            ...
            if probe != nil {
                Grid(alignment: .leading, horizontalSpacing: Space.md, verticalSpacing: Space.md) {
                    ...
                    GridRow {
                        ...
                        TextField("HEAD", text: $baseRef).textFieldStyle(.roundedBorder).font(.btMono)
                    }
                    GridRow {
