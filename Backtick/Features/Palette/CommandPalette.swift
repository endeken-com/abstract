...
struct CommandPalette: View {
        ...
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 0) {
                HStack(spacing: Space.md) {
                    Image(systemName: "magnifyingglass").font(.system(size: 15)).foregroundStyle(Color.btTextTertiary)
    ...
    private func row(_ item: Item, highlighted: Bool) -> some View {
        HStack(spacing: Space.md) {
            Group {
                ...
                else if let s = item.symbol { Image(systemName: s).font(.system(size: 13)).foregroundStyle(Color.btTextSecondary) }
            }
            .frame(width: 18)
    ...
    private var all: [Item] {
        ...
        if let s = model.selectedSession {
            items.append(Item(id: "review", section: s.name, title: model.chatTab == .changes ? "Show conversation" : "Review changes", symbol: "plusminus", shortcut: ["⌘", "D"]) {
