...
/// Backtick's button styles. Each reacts to hover and press, and dims when
    ...
    private struct StyledButton: View {
        ...
        var body: some View {
            configuration.label
                ...
                .overlay {
                    if kind == .secondary {
                        RoundedRectangle(cornerRadius: size == .small ? 6 : 8, style: .continuous)
                            .strokeBorder(Color.btBorderStrong, lineWidth: 0.5)
                    }
                }
        ...
        private var background: AnyShapeStyle {
            switch kind {
            case .primary:
                AnyShapeStyle(Color.accentColor.opacity(configuration.isPressed ? 0.85 : hovering ? 0.92 : 1))
            case .secondary:
                AnyShapeStyle(hovering ? Color.btInset : Color.btSurfaceRaised)
            case .ghost:
