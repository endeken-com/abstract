...
/// Fires automations on schedule while Backtick runs. Closing the window
...
final class AutomationScheduler {
    ...
    private func tick() async -> TimeInterval {
        ...
        for a in model.automations where a.enabled {
            let next = a.nextRunAt ?? (try? Schedule.nextOccurrence(rrule: a.rrule, timezone: a.timezone, dtstart: a.dtstart, after: now)) ?? nil
            guard let due = next else { continue }
            var nextRun = due
            if due <= now {
                ...
                guard let after = (try? Schedule.nextOccurrence(rrule: a.rrule, timezone: a.timezone, dtstart: a.dtstart, after: now)) ?? nil
                else { recordNextRun(nil, for: a.id); continue }
                nextRun = after
            }
            recordNextRun(nextRun, for: a.id)
    ...
    /// an edit saved while a run was firing is never overwritten by a stale one.
    private func recordNextRun(_ date: Date?, for id: String) {
        guard var fresh = (try? model.store.automation(id)) ?? nil, fresh.enabled else { return }
        guard fresh.nextRunAt != date else { return }
